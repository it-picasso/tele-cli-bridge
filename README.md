...
- **Sandboxed subprocesses.** Every CLI invocation runs in an isolated tmp directory whose only project memory is the security-instructions file (`CLAUDE.md` / `GEMINI.md`). User input is passed via `ProcessBuilder` argv (no shell). Read-only modes are forced (`--permission-mode plan` for Claude, `--approval-mode plan` for Gemini), and built-in tools are disabled (`--tools ""` for Claude).
